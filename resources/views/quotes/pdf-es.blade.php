...
<body>
    <div class="header">
        <div class="header-grid">
            <div class="header-left">
                ...
                <p><strong>Fecha:</strong> {{ $quote->quote_date->format('d/m/Y') }}</p>
                @if($quote->valid_until)
                <p><strong>Válido hasta:</strong> {{ $quote->valid_until->format('d/m/Y') }}</p>
                @endif
            </div>
    ...
    <div class="section">
        ...
        <div class="details">
            ...
            @if($quote->customer_tax_id)
                <strong>NIF/CIF:</strong> {{ $quote->customer_tax_id }}<br>
            @endif
        </div>
    </div>
    @endif
        ...
        <table>
            @if($showTaxColumn)
                ...
                @if($quote->irpf_rate && $quote->irpf_amount)
                    <tr>
                        <td>Retención IRPF {{ rtrim(rtrim(number_format((float) $quote->irpf_rate, 2, ',', '.'), '0'), ',') }}%:</td>
    ...
    <div class="accepted-info">
        <div class="accepted-stamp">ACEPTADO / ACCEPTED</div>
    </div>
    @endif
    ...
    <div class="footer">
        ...
        <p>Presupuesto generado el {{ $quote->generated_at?->format('d/m/Y H:i') ?? now()->format('d/m/Y H:i') }}</p>
    </div>
</body>
</html>
