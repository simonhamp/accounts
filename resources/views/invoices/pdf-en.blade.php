...
<body>
    ...
    @endphp
    <div class="section">
        <div class="section-title">{{ $invoice->isCreditNote() ? 'CREDIT NOTE DETAILS' : 'INVOICE DETAILS' }}</div>
        <table>
            <thead>
                <tr>
        ...
        <table>
            @if($showTaxColumn)
                ...
                @if($invoice->irpf_rate && $invoice->irpf_amount)
                    <tr>
                        <td>IRPF Withholding {{ rtrim(rtrim(number_format((float) $invoice->irpf_rate, 2, '.', ','), '0'), '.') }}%:</td>
    ...
    <div class="payment-page">
        ...
        <div class="amount-due">
            ...
            @if($invoice->due_date)
                <div class="label" style="margin-top: 10px;">Due by {{ $invoice->due_date->format('j F Y') }}</div>
            @else
                <div class="label" style="margin-top: 10px;">Due on receipt</div>
            @endif
        </div>
        ...
        <div class="reference">
            ...
            <div class="label" style="margin-top: 5px;">Please use this reference when making payment</div>
        </div>

        <div class="notice" style="margin-top: 30px;">
            <strong>Important:</strong> Please pay the exact amount shown above. Any bank fees or transfer charges should be paid by the sender.
        </div>
    </div>
    @endif
</body>
