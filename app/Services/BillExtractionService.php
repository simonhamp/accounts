...
class BillExtractionService
{
    ...
    {
        return 'You are an expert at extracting structured data from supplier invoices/bills. You are thorough, accurate, and follow instructions precisely. When you find information, you extract it even if you are not 100% certain.';
    }

    protected function getTextExtractionPrompt(string $pdfText): string
    {
        return <<<PROMPT
...
PROMPT;
    ...
    protected function getImageExtractionPrompt(): string
    {
        return <<<'PROMPT'
I am sharing an image of a supplier invoice/bill/receipt. This is a bill we RECEIVED from a supplier (not an invoice we issued). Please extract all information and return it as JSON with the following structure:
...
- Items array should contain all line items from the bill
- When in doubt, EXTRACT rather than skip - we can verify manually later
- For receipts that don't have detailed line items, create a single item with the total amount
PROMPT;
    }
}
